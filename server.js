...
`;
    ...
    // Persist analysis to sqlite DB (can be disabled for fast prototype)
    try {
      if (process.env.DISABLE_PERSISTENCE === 'true') {
        log('Persistence disabled (DISABLE_PERSISTENCE=true) — analysis not saved');
      } else {
...
app.post('/api/request-consult', (req, res) => {
  ...
  // Persist consult request (can be disabled for fast prototype)
  try {
    if (process.env.DISABLE_PERSISTENCE === 'true') {
      log('Persistence disabled (DISABLE_PERSISTENCE=true) — consult not saved');
    } else {
      const rec = {
        id: requestId,
        name,
